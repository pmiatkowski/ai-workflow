...
```
Bug '{name}' not found at .ai-workflow/bugs/{name}/

Available bugs: {list available bugs from .ai-workflow/bugs/}
...
Check `.ai-workflow/bugs/{name}/fix-plan.md` exists.
...
Read from `.ai-workflow/bugs/{name}/`:
...
Update `.ai-workflow/bugs/{name}/state.yml`:
...
Read `.ai-workflow/config.yml` and extract `workflows.verification.commands`.
...
After all tasks are complete and verification passes, update `.ai-workflow/bugs/{name}/state.yml`:
...
- `.ai-workflow/memory/global-state.yml` → confirms current is bug, name=login-timeout
- `.ai-workflow/bugs/login-timeout/state.yml` → status=fixing
- `.ai-workflow/bugs/login-timeout/fix-plan.md` → has 5 tasks
